...
export const MB: Record<AppLocale, MbUiStrings> = {
  ...
  he: {
    ...
    infoMethodology:
      ...
      'הערת דאטה (חלון בסיס 30 יום, צילום נוכחי של הגיליון): ב־87.8% מהסקרים (468/533) אין לסוקר אף סקר נוסף מערוץ אחר בתוך חלון הבסיס של אותו סקר. סקר זהה באותו יום — עם אותו סוקר מוצג תחת שני ערוצים — זוהה פעם אחת בלבד במיצג הזה; אין כאן זיהום דומיננטי של כפילויות מסוג זה.\n\n' +
      'שיטת ה־LOO מוציאה מהבייסליין את הגוף המפרסם עצמו — לא את השם „סוקר” באופן גלובלי — כדי שלא ימשוך אליו את הממוצע שנבנה לו. המודל מודד חריגה יחסית מקונצנזוס השוק („עדריות”), ולא קובע „אמת” מול הקלפי.',
