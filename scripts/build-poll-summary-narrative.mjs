...
function partyBulletEn(partyKey, avg, nOutlets) {
  ...
  if (partyKey === "Ofer Winter's Party") {
    return `[[party:Winter]]${partyLabelWrap(`<strong>${partyDisplayEn(partyKey)}</strong>`)} – ${clause}`
  }
...
function partyBulletHe(partyKey, avg, nOutlets, partyHebByKey) {
  ...
  if (partyKey === "Ofer Winter's Party") {
    return `[[party:Winter]]${partyLabelWrap(`<strong>${partyDisplayHe(partyKey, partyHebByKey)}</strong>`)} – ${clause}`
  }
  ...
  if (fs.existsSync(outPath)) {
    try {
      ...
      for (const loc of ['en', 'he']) {
        const bg = (existing.locales?.[loc]?.background ?? '').trim()
        if (bg) doc.locales[loc].background = bg
      }
    } catch {
      /* keep defaults from buildPollSummaryNarrativeDoc */
    }
  }
